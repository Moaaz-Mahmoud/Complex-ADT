...
//Complex operators (Complex with other datatypes) 
...
Complex* Complex::root(const int k) {
	...
	for (int i = 0; i < k; i++) {
		results[i].set(
			pow(this->get_magnitude(),
				1 / static_cast<float>(k)) *
				cos((this->rad_angle() + 2 *
				PI * (i - 1)) / static_cast<float>(k)),
			pow(this->get_magnitude(),
				1 / static_cast<float>(k)) *
			sin((this->rad_angle() + 2 * PI *
			(i - 1)) / static_cast<float>(k))
		);
